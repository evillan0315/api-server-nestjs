...
export class GoogleGeminiService {
  ...
  async processInputAndSaveToDb(userId: string, question: string, chatId: string): Promise<any> {
    ...
    if (existingChat) {
      ...
      apiUsage = await this.prisma.apiUsage.create({
        data: {
          ...
          inputTokens: generateResult?.response?.usageMetadata?.candidatesTokenCount, // Example data from the API
          outputTokens: generateResult?.response?.usageMetadata?.totalTokenCount, // Example data from the API
      ...
      chat = await this.prisma.chat.create({
        data: {
          userId: "26833955-08a2-4d40-bfcd-3cffcc10aca4",
        },
      });
      if(chat.id){
      chatId = chat.id;
